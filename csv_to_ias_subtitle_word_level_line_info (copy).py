...
def sniff_delimiter(path: Path) -> str:
    """
    Your pasted sample looks TSV. This tries to auto-detect between tab and comma.
    """
...
def load_master_rows(master_path: Path) -> List[Dict[str, str]]:
    """
    Load master rows in order. Keeps duplicates.
    Accepts TSV or CSV.
    """
...
def load_csv_group_by_sentence(csv_path: Path) -> Dict[str, List[dict]]:
    """
    Group rows only when sentence text (or a similar variant) is the same consecutively.
    Produces keys like "<base>__groupN".
    """
...
def adjust_y_per_sentence(records: List[WordRecord]) -> List[WordRecord]:
    ...
    for _, idxs in span_to_indices.items():
        ...
        for idx in idxs:
            line_idx = int(new_records[idx][9])  # line_idx at index 9
...
def main():
    ...
    write_ias_word_level(
        groups, word_out_path, fps=args.fps, negative_times=negative_times,
        master_rows=master_rows, lookahead=args.master_lookahead, match_threshold=args.master_threshold
    )
    write_ias_sentence_level(
        groups, sentence_out_path, fps=args.fps, negative_times=negative_times,
        master_rows=master_rows, lookahead=args.master_lookahead, match_threshold=args.master_threshold
    )
